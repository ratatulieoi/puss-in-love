...
function Browse({ token }) {
    ...
    const [browseCats, setBrowseCats] = useState([]);
    ...
    const fetchBrowseCats = async (catId) => {
        if (!catId) return;
        setMsg('');
        setError('');
        setIndex(0);

        const res = await fetch(`${API}/cats/browse?owner_cat_id=${catId}`, { headers });
        const data = await res.json();
        if (!res.ok) return setError(data.error);
        setBrowseCats(data.data);
    };

    useEffect(() => { fetchMyCats(); }, []);

    useEffect(() => {
        if (ownerCatId) fetchBrowseCats(ownerCatId);
    }, [ownerCatId]);

    const swipe = async (direction) => {
        const target = browseCats[index];
        if (!target) return;
    ...
    const current = browseCats[index];
    ...
    return (
        <div>
            ...
            {myCats.length === 0 ? (
                <p className="card">Add your own cat first before browsing.</p>
            ) : (
                <div className="card">
                    ...
                    <button className="btn-secondary" onClick={() => fetchBrowseCats(ownerCatId)}>Refresh Browse</button>
                </div>
            )}
